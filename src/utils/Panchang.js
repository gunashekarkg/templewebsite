...
const SAMVATSARA = [
  ...
  "Soumya", "Sadharana", "Virodhikrit", "Paridhavi", "Pramadi", "Ananda", 
  "Rakshasa", "Nala", "Pingala", "Kalayukti", "Siddharthi", "Raudra", 
  "Durmathi", "Dundubhi", "Rudhirodgari", "Raktakshi", "Krodhana", "Kshaya"
];
...
/*function moonRiseSet(jd, lat, lon) {
  const step = 1 / 96; // ~15 minutes
  const sinLat = Math.sin(lat * Math.PI / 180);
  const cosLat = Math.cos(lat * Math.PI / 180);
  let rise = null, set = null, prevAlt = null;

  for (let h = -0.5; h <= 1.5; h += step) {
    const testJd = jd + h;
    const L = moon(testJd);
    const decl = Math.asin(Math.sin(L * d2r) * Math.sin(5.145 * d2r));
    const sinAlt = sinLat * Math.sin(decl) + cosLat * Math.cos(decl);
    const alt = Math.asin(sinAlt) * r2d;

    if (prevAlt !== null) {
      if (alt > 0 && prevAlt <= 0 && !rise) rise = calData(testJd);
      if (alt < 0 && prevAlt >= 0 && !set) set = calData(testJd);
    }
    prevAlt = alt;
  }
...
class Panchang {
  ...
  getMoonAltitude(jd, latitude, longitude) {
    const L = this.degToRad(moon(jd)); // Moon longitude
    const B = 0; // Approximate moon latitude = 0 (simplified)
    const ε = this.degToRad(23.4397); // Obliquity of the ecliptic

    const sinDec = Math.sin(ε) * Math.sin(L);
    const dec = Math.asin(sinDec);

    const lst = this.calculateSiderealTime(jd, longitude, 0);
    const ha = this.degToRad(lst - this.radToDeg(L)); // Hour angle
    ...
    return this.radToDeg(Math.asin(sinAlt));
  }
  ...
  // Solar Tithi Calculation
  calculateSolarTithi(jd) {
    const sunLong = sun(jd);
    const solarTithi = Math.floor(((sunLong % 360) / 12)) + 1;
    const month = Math.floor(((sunLong % 360) / 30));
    return { tithi: solarTithi, month: month };
  }

  // Samvat Year Systems
  calculateYearSystems(gregYear) {
    return {
      vikram: gregYear + 57,
      saka: gregYear - 78,
      kali: gregYear + 3101,
      gujarati: gregYear + 56
    };
  }

  // Choghadiya and Hora
  calculateChoghadiya(sunrise, sunset) {
    const slots = ['Shubh','Rog','Udveg','Char','Labh','Amrit','Kaal','Shubh'];
    const choghadiyas = [];
    const dayDuration = (sunset - sunrise) / 8;
    for (let i = 0; i < 8; i++) {
      choghadiyas.push({
        name: slots[i],
        start: new Date(sunrise.getTime() + i * dayDuration),
        end: new Date(sunrise.getTime() + (i + 1) * dayDuration)
      });
    }
    return choghadiyas;
  }

  calculateHora(sunrise) {
    const planets = ['Surya', 'Shukra', 'Budha', 'Chandra', 'Shani', 'Guru', 'Mangal'];
    const horas = [];
    for (let i = 0; i < 12; i++) {
      horas.push({
        ruler: planets[i % 7],
  ...
  calculateYoga(jd, tzone) {
    const ayan = calcayan(jd);
    const sum = fix360(moon(jd) + sun(jd) - 2 * ayan);
    const index = Math.floor(sum / (360 / 27));
    const target = (index + 1) * (360 / 27);

    let jdt = jd;
    let iter = 0;
    while (iter++ < 50) {
      const a = calcayan(jdt);
      const m = fix360(moon(jdt) - a);
      const s = fix360(sun(jdt) - a);
      const angle = fix360(m + s);
      const delta = fix360(target - angle);
      if (delta < 0.01) break;
      jdt += delta / 13.2;
    }
    ...
    return {
      name: yog[index],
      start: this.julianToDate(jd + (tzone / 24)),
      end: this.julianToDate(jdt + (tzone / 24))
    };
  }

  calculatePaksha(jd) {
    const moonT = fix360(moon(jd));
    const sunT = fix360(sun(jd));
    const diff = fix360(moonT - sunT);
    //return diff < 180 ? PAKSHA[0] : PAKSHA[1]; // Krishna or Shukla
    return diff < 180 ? PAKSHA[1] : PAKSHA[0];
  }
  ...
  calculateSamvatsara(jd) {
    const t = (jd - 2415020) / 365.25;
    const index = Math.floor((t + 9) % 60);
    return SAMVATSARA[index];
  }

  calculateSuryaNakshatra(jd) {
    const ayan = calcayan(jd);
    const Lsun = fix360(sun(jd) - ayan);
    const index = Math.floor(Lsun / (360 / 27));
    const startLong = index * (360 / 27);
    const endLong = (index + 1) * (360 / 27);
  ...
  calculateKarana(jd, tzone) {
    const tithiObj = this.calculateTithi(jd, tzone);
    const tIndex = tith.findIndex(t => t === tithiObj.name);
    const diff = fix360(moon(jd) - sun(jd));
    const index = Math.floor((tIndex * 2 + (diff % 12) / 6)); // 2 Karanas per Tithi

    let karanaIndex;
    if (tIndex === 0) karanaIndex = 10;       // Kimstughna
    else if (tIndex === 29) karanaIndex = 11; // Shakuni
    else if (tIndex === 14) karanaIndex = 9;  // Chatushpada
    else if (tIndex === 15) karanaIndex = 8;  // Naga
    else karanaIndex = (index - 1) % 7;

    return {
      name: kar[karanaIndex],
      start: tithiObj.start,
      end: tithiObj.end
    };
  }

  calculateLagna(jd, latitude, longitude, tzone) {
    // Simplified lagna calculation - would need more complex implementation
    const siderealTime = this.calculateSiderealTime(jd, longitude, tzone);
    const lagnaIndex = Math.floor(((siderealTime % 24) / 2) % 12);
    const degree = (siderealTime % 2) * 30;
    return {
      sign: zn[lagnaIndex],
      degree
    };
  ...
  calculateSiderealTime(jd, longitude, tzone) {
    // Calculate local sidereal time
    const t = (jd - 2451545.0) / 36525.0;
    let gmst = 280.46061837 + 360.98564736629 * (jd - 2451545.0) +
      0.000387933 * t * t - t * t * t / 38710000.0;
    gmst = fix360(gmst);
    return fix360(gmst + longitude - tzone * 15) / 15;
  }
  ...
  calculateSunPosition(jd) {
    ...
    const lambda = L + 1.915 * Math.sin(this.degToRad(g)) + 0.020 * Math.sin(this.degToRad(2 * g));
    const epsilon = 23.439 - 0.0000004 * n;
    
    const alpha = this.radToDeg(Math.atan2(
      Math.cos(this.degToRad(epsilon)) * Math.sin(this.degToRad(lambda)),
      Math.cos(this.degToRad(lambda))
    ));
    
    const delta = this.radToDeg(Math.asin(
      Math.sin(this.degToRad(epsilon)) * Math.sin(this.degToRad(lambda))
    ));
    
    return { alpha, delta };
  }

  calculateSunriseSunset(jd, latitude, longitude, zenith = 90.8333) {
    const latRad = this.degToRad(latitude);
    const lngHour = longitude / 15;
    const sunPos = this.calculateSunPosition(jd);
    const deltaRad = this.degToRad(sunPos.delta);
    const cosH = (Math.cos(this.degToRad(zenith)) / (Math.cos(latRad) * Math.cos(deltaRad)) -
                  Math.tan(latRad) * Math.tan(deltaRad));

    if (cosH > 1 || cosH < -1) return { sunrise: null, sunset: null };

    const H = this.radToDeg(Math.acos(cosH));
    const riseJD = jd - (H + sunPos.alpha - 15 * lngHour) / 360;
    const setJD = jd + (H - sunPos.alpha + 15 * lngHour) / 360;

    return {
      sunrise: this.julianToDate(riseJD),
      sunset: this.julianToDate(setJD)
    };
  };
  ...
  calculateSunTimes(date, latitude, longitude, tzone) {
    const times = SunCalc.getTimes(date, latitude, longitude);

    // Don't manually add timezone offset here
    // Just convert UTC to local directly
    /*if (times.sunrise) {
    //times.sunrise.setHours(times.sunrise.getHours() + tzone);
    times.sunrise = new Date(times.sunrise.getTime() + tzone * 3600000);
    }
    if (times.sunset) {
      //times.sunset.setHours(times.sunset.getHours() + tzone);
      times.sunset = new Date(times.sunset.getTime() + tzone * 3600000);
    }
    return times;*/
    return {
      sunrise: DateTime.fromJSDate(times.sunrise).toJSDate(),
      sunset: DateTime.fromJSDate(times.sunset).toJSDate()
    };
  }

  calculateInauspiciousTimes(sunrise, sunset, weekday) {
    if (!sunrise || !sunset) {
      return {
    ...
    const rahuSegments = [
      7, 1, 6, 5, 4, 3, 2 // Sunday to Saturday indexes
    ];
    const yamaSegments = [
      5, 6, 4, 3, 2, 1, 7
    ];
    const gulikaiSegments = [
      6, 5, 7, 4, 3, 2, 1
    ];
    const rahuIndex = rahuSegments[weekday];
    const yamaIndex = yamaSegments[weekday];
    const gulikaiIndex = gulikaiSegments[weekday];

    const getPeriod = (index) => ({
      start: new Date(sunrise.getTime() + segment * (index - 1)),
      end: new Date(sunrise.getTime() + segment * index)
    });

    return {
      rahukalam: getPeriod(rahuIndex),
      yamagandam: getPeriod(yamaIndex),
      gulikai: getPeriod(gulikaiIndex)
    };
  }

  calculateAbhijitMuhurta(sunrise, sunset) {
    if (!sunrise || !sunset || isNaN(sunrise.getTime()) || isNaN(sunset.getTime()) || sunrise >= sunset) {
      return { start: null, end: null };
    }

    const duration = sunset - sunrise;
    const midpoint = new Date(sunrise.getTime() + duration / 2);
    const offset = duration / 15;

    return {
      start: new Date(midpoint.getTime() - offset),
      end: new Date(midpoint.getTime() + offset)
    };
  }

  /*calculateMoonRiseSet(jd, latitude, longitude) {
    const step = 1 / 96; // ~15 minutes
    const horizon = -0.3;
    let prevAlt = null;
    let moonrise = null;
    ...
    for (let h = -0.5; h < 1.5; h += step) {
      ...
      const L = moon(t);
      const decl = Math.asin(Math.sin(L * d2r) * Math.sin(5.145 * d2r));
      const sinLat = Math.sin(latitude * d2r);
      const cosLat = Math.cos(latitude * d2r);
      const sinAlt = sinLat * Math.sin(decl) + cosLat * Math.cos(decl);
      const alt = Math.asin(sinAlt) * r2d;

      if (prevAlt !== null) {
        if (alt > horizon && prevAlt <= horizon && !moonrise) {
          moonrise = calData(t);
        }
        if (alt < horizon && prevAlt >= horizon && !moonset) {
          moonset = calData(t);
        }
      }
      prevAlt = alt;
  ...
  calculateMoonRiseSet(date, latitude, longitude) {
    const times = SunCalc.getMoonTimes(date, latitude, longitude);

    return {
      moonrise: times.rise ? DateTime.fromJSDate(times.rise).toJSDate() : null,
      moonset: times.set ? DateTime.fromJSDate(times.set).toJSDate() : null
    };
  }

  calculateTithi(jd, tzone) {
    const ayan = calcayan(jd);
    const Lmoon = fix360(moon(jd) - ayan);
    const Lsun = fix360(sun(jd) - ayan);
    const diff = fix360(Lmoon - Lsun);
    const index = Math.floor(diff / 12);
    const target = (index + 1) * 12;
  ...
  calculateNakshatra(jd, tzone) {
    const ayan = calcayan(jd);
    const Lmoon = fix360(moon(jd) - ayan);
    const index = Math.floor(Lmoon / (360 / 27));
    const target = (index + 1) * (360 / 27);
  ...
  calculateAyana(jd) {
    const sunLong = sun(jd);
    // When Sun is in Makara to Mithuna, it's Uttarayana; else Dakshinayana
    const sign = Math.floor(sunLong / 30); // 0 = Mesha, 1 = Vrishabha, ..., 11 = Meena
    return (sign >= 9 || sign < 3) ? "Uttarayana" : "Dakshinayana";
  }

  calculateZodiac(degree) {
    return zn[Math.floor(((degree % 360) + 360) % 360 / 30) % 12];
  }

  calculateDayNightLength(sunrise, sunset) {
    if (
      !sunrise || !sunset ||
      isNaN(sunrise.getTime()) || isNaN(sunset.getTime())
    ) {
      return { day: 'NaN:NaN:NaN', night: 'NaN:NaN:NaN' };
    }

    const duration = Math.abs(sunset - sunrise);         // in ms
    const night = 86400000 - duration;                   // 24h - day length
  ...
  /*calculateDayNightLength(sunrise, sunset) {
    ...
    if (sunset < sunrise) [sunrise, sunset] = [sunset, sunrise];
    const dayMillis = Math.abs(sunset.getTime() - sunrise.getTime());
    const nightMillis = 86400000 - dayMillis;
    const toHMS = millis => {
  ...
  calculate(d, latitude = 53.755, longitude = 9.656, timezone = 'Europe/Berlin') {
    // Normalize timezone and tzone
    let tzone;
    if (typeof timezone === 'number') {
      ...
      try {
        ...
        tzone = -localDate.getTimezoneOffset() / 60;
      } catch (e) {
        console.warn("Invalid timezone string, defaulting to UTC");
        timezone = 'UTC';
        tzone = 0;
      }
    } else {
    ...
    //const tzone = -d.getTimezoneOffset() / 60; // for JD calculations
    //const timezone = timezone; // for formatting
    ...
    // New calculations
    this.Paksha = this.calculatePaksha(jd);
    this.Ritu = this.calculateRitu(jd);
    this.Samvatsara = this.calculateSamvatsara(jd);
    this.SuryaNakshatra = this.calculateSuryaNakshatra(jd);
    //this.Raasi.sun = this.calculateZodiac(Lsun);
    //this.Raasi.moon = this.calculateZodiac(Lmoon);
    this.Ayana = this.calculateAyana(jd);
    this.Raasi.sun = this.calculateZodiac(Lsun + ayanamsa); // sidereal
    this.Raasi.moon = this.calculateZodiac(Lmoon + ayanamsa);

    
        
    // Calculate sun times and related periods
    const sunTimes = this.calculateSunTimes(d, latitude, longitude, timezone);
    this.SunTimes = sunTimes;
    
    //this.DayLength = this.calculateDayNightLength(sunTimes.sunrise, sunTimes.sunset);
    this.AuspiciousTimes.abhijit = this.calculateAbhijitMuhurta(this.SunTimes.sunrise, this.SunTimes.sunset);
    this.DayLength = this.calculateDayNightLength(this.SunTimes.sunrise, this.SunTimes.sunset);

    this.MoonTimes = this.calculateMoonRiseSet(d, latitude, longitude);

    this.InauspiciousTimes = this.calculateInauspiciousTimes(
      sunTimes.sunrise, sunTimes.sunset, n_wday
    );
    
    /**this.AuspiciousTimes = {
      abhijit: this.calculateAbhijitMuhurta(sunTimes.sunrise, sunTimes.sunset)
    };**/
    
    this.Lagna = this.calculateLagna(jd, latitude, longitude, tzone);
    ...
    const nakshData = this.calculateNakshatra(jd, tzone);
    this.Nakshatra.name = nakshData.name;
    this.Nakshatra.start = nakshData.start;
    this.Nakshatra.end = nakshData.end;
    this.Nakshatra = nakshData;

    /*this.Karna.name = kar[n_karana];
    this.Karna.start = s_karana.start;
    this.Karna.end = s_karana.end;*/
    this.Karna = this.calculateKarana(jd, tzone);
    ...
    const tithiData = this.calculateTithi(jd, tzone);
    this.Tithi.name = tithiData.name;
    this.Tithi.start = tithiData.start;
    this.Tithi.end = tithiData.end;
    this.Tithi = tithiData;

    this.Yoga = this.calculateYoga(jd, tzone);
    this.LagnaTimings = this.calculateLagnaTimings(jd0, latitude, longitude, tzone);
    this.SolarTithi = this.calculateSolarTithi(jd);
    this.SamvatYears = this.calculateYearSystems(year);
    this.Choghadiya.day = this.calculateChoghadiya(this.SunTimes.sunrise, this.SunTimes.sunset);
    this.Hora.day = this.calculateHora(this.SunTimes.sunrise);

    // Simple festival detection - would need more complex implementation
    if (this.Tithi.name === "Ekadasi") {
      this.Festivals.push("Ekadasi Vrata");
    }
    if (this.Paksha === "Shukla Paksha" && this.Tithi.name === "Purnami") {
      this.Festivals.push("Purnima Vrata");
    }
  }
}
