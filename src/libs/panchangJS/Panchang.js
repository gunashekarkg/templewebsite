...
class Panchang {
  ...
  calculateSuryaNakshatra(jd) {
    const sunLong = sun(jd) + ayanamsa;
    const nakshIndex = Math.floor((sunLong * 6) / 80);
    return {
      name: naks[nakshIndex],
      start: null, // These would need actual calculation
      end: null
    };
  }
  ...
  calculateLagna(jd, latitude, longitude, tzone) {
    ...
    const lagnaIndex = Math.floor(siderealTime / 2) % 12;
    return {
      sign: zn[lagnaIndex],
  ...
  calculateSiderealTime(jd, longitude, tzone) {
    ...
    gmst = fix360(gmst);
    return fix360(gmst + longitude - tzone * 15);
  }
  ...
  calculateSunriseSunset(jd, latitude, longitude, zenith = 90.8333) {
    ...
    if (cosH > 1 || cosH < -1) {
      return { sunrise: null, sunset: null }; // No sunrise/sunset (polar day/night)
    }
    
    const H = this.radToDeg(Math.acos(cosH));
    
    // Calculate Julian dates for sunrise/sunset
    const sunriseJD = jd + (360 - H - sunPos.alpha + 15 * lngHour) / 360;
    const sunsetJD = jd + (H - sunPos.alpha + 15 * lngHour) / 360;
    
    return {
      sunrise: this.julianToDate(sunriseJD),
      sunset: this.julianToDate(sunsetJD)
    };
  }
  ...
  calculateSunTimes(jd, latitude, longitude, tzone) {
    const times = this.calculateSunriseSunset(jd, latitude, longitude);
  ...
  calculateInauspiciousTimes(sunrise, sunset, weekday) {
    ...
    // Rahukalam varies by weekday (1.5 hour period)
    const rahuWeekdayMap = [
      [16.5, 18],   // Sunday
      [7.5, 9],      // Monday
      [15, 16.5],    // Tuesday
      [12, 13.5],    // Wednesday
    ...
    const [rahuStartHour, rahuEndHour] = rahuWeekdayMap[weekday];
    const rahuStart = new Date(sunrise);
    rahuStart.setHours(rahuStart.getHours() + rahuStartHour);
    const rahuEnd = new Date(sunrise);
    rahuEnd.setHours(rahuEnd.getHours() + rahuEndHour);
    
    // Yamagandam (1.5 hours after sunrise)
    const yamaStart = new Date(sunrise);
    yamaStart.setHours(yamaStart.getHours() + 3);
    const yamaEnd = new Date(yamaStart);
    yamaEnd.setHours(yamaEnd.getHours() + 1.5);
    
    // Gulikai (4.5 hours after sunrise)
    const gulikaiStart = new Date(sunrise);
    gulikaiStart.setHours(gulikaiStart.getHours() + 4.5);
    const gulikaiEnd = new Date(gulikaiStart);
    gulikaiEnd.setHours(gulikaiEnd.getHours() + 1.5);
  ...
  calculateAbhijitMuhurta(sunrise, sunset) {
    ...
    const abhijitStart = new Date(midday.getTime() - 24 * 60 * 60 * 1000);
    const abhijitEnd = new Date(midday.getTime() + 24 * 60 * 60 * 1000);
  ...
  calculate(d, latitude = 53.755, longitude = 9.656, timezone = 'Europe/Berlin') {
    ...
    const sunTimes = this.calculateSunTimes(jd, latitude, longitude, tzone);
